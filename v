...
if not success or not Sirius then
    warn("Failed to load Rayfield UI Library: " .. tostring(Sirius))
    return
end

-- Create UI
local Window = Sirius:CreateWindow({Name = "AutoParry Menu", Theme = "Dark", Size = UDim2.new(0, 400, 0, 300)})
local MainTab = Window:CreateTab("Main")
local ConfigTab = Window:CreateTab("Config")

-- UI Elements
MainTab:CreateToggle({Name = "Enable AutoParry", Default = false, Callback = function(value) isRunning = value end})
MainTab:CreateToggle({Name = "Auto Attack", Default = true, Callback = function(value) autoAttack = value end})
MainTab:CreateToggle({Name = "Auto Ability", Default = true, Callback = function(value) autoAbility = value end})
